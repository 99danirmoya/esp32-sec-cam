...
// ── Detección ───────────────────────────────────────────────────────
const long MOTION_THRESHOLD = 200000;
...
void clearHighResState(bool uploadSuccess) {
  if (xSemaphoreTake(stateMutex, portMAX_DELAY)) {
    ...
    if (uploadSuccess) {
      ...
      Serial.println("-> Cooldown HR activado: 10 s");
    }
...
void checkCooldownFinished() {
  if (xSemaphoreTake(stateMutex, portMAX_DELAY)) {
    if (!cooldownEndReported &&
      ...
      Serial.println("-> Cooldown HR finalizado. Sistema listo para nueva captura.");
...
void applyGlobalStaticIP() {
  if (!WiFi.config(fixed_local_IP, fixed_gateway, fixed_subnet, fixed_primaryDNS, fixed_secondaryDNS)) {
    Serial.println("No se pudo configurar la IP fija");
  } else {
    Serial.printf("IP fija configurada: %s\n", fixed_local_IP.toString().c_str());
  }
}
...
bool connectWiFiWithManager(bool forcePortal = false) {
  ...
  if (!ok) {
    Serial.println("WiFiManager no pudo conectar/configurar WiFi");
    return false;
  }

  Serial.printf("WiFi conectada: %s\n", WiFi.SSID().c_str());
...
void ensureWiFiConnected() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    Serial.printf("WiFi reconectada: %s  IP: %s\n",
      WiFi.SSID().c_str(),
      WiFi.localIP().toString().c_str());
    return;
  }

  Serial.println("Reconexión fallida. Lanzando WiFiManager...");
...
void configureMqttByCurrentSSID() {
  ...
  if (currentSSID == "Wifioficina") {
    ...
    Serial.println("MQTT modo oficina");
  } else {
    mqttClient.setServer(mqtt_server_remote, mqtt_port_remote);
    Serial.println("MQTT modo remoto");
  }
}
...
bool captureHighResColor(uint8_t **outBuf, size_t *outLen) {
  ...
  *outLen = 0;
  ...
  if (fbDiscard) {
    ...
    Serial.println("Primer frame de descarte fallido");
  }
  ...
  *outLen = fb->len;
  ...
  Serial.printf("Captura color valida: %u bytes\n", (unsigned)*outLen);
...
void setup() {
  ...
  if (!connectWiFiWithManager(false)) {
    Serial.println("No se pudo conectar/configurar WiFi");
  }
